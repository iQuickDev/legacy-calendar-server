...
$hostLogin = Invoke-RestMethod -Uri "http://localhost:3000/auth/login" -Method Post -Body $loginPayload -Headers @{ "X-Bypass" = $bypassKey } -ContentType "application/json" -ErrorAction Stop
...
$eventRes = Invoke-RestMethod -Uri "http://localhost:3000/events" -Method Post -Body $eventPayload -Headers @{ Authorization = "Bearer $hostToken"; "X-Bypass" = $bypassKey } -ContentType "application/json" -ErrorAction Stop
$eventId = $eventRes.id
...
Invoke-RestMethod -Uri "http://localhost:3000/users" -Method Post -Body $guestPayload -Headers @{ "X-Bypass" = $bypassKey } -ContentType "application/json" -ErrorAction Stop

# 5. Login Guest
Write-Host "`n5. Logging in Guest..."
$guestLoginPayload = @{ username = $guestUser; password = $pass } | ConvertTo-Json
$guestLogin = Invoke-RestMethod -Uri "http://localhost:3000/auth/login" -Method Post -Body $guestLoginPayload -Headers @{ "X-Bypass" = $bypassKey } -ContentType "application/json" -ErrorAction Stop
$guestToken = $guestLogin.access_token

# 6. Join Event
Write-Host "`n6. Guest joining event..."
$joinRes = Invoke-RestMethod -Uri "http://localhost:3000/events/$eventId/join" -Method Post -Headers @{ Authorization = "Bearer $guestToken"; "X-Bypass" = $bypassKey } -ContentType "application/json" -ErrorAction Stop
